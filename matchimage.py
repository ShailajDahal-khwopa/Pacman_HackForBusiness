...
def extract_keyword_from_image_easyocr(img):
    ...
    words = [text[1].strip().lower() for text in result if len(text[1]) > 2]
    if not words:
        print("⚠️ No useful keyword found. Please enter manually.")
        return input("Enter product keyword manually: ")
    return words[0]  # use best guess

# -------- STEP 3: Web scraping from Khadyanna --------
...
# -------- STEP 4: Load pretrained ResNet50 for feature extraction --------
def init_model():
